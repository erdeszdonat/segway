    ...
    <div className="min-h-screen relative flex items-center justify-center p-2 font-sans bg-cover bg-fixed transition-all duration-700" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <button type="submit" disabled={isCheckingCity}
                ...
                {isCheckingCity ? "Várjon..." : "Ideális gép mutatása"}
              </button>
            </div>
          </div>
        </form>
      ...
      {eredmeny && !isAnimating && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/85 backdrop-blur-md p-4 overflow-y-auto">
          <div className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden my-auto max-h-[90vh] flex flex-col border border-white/20">
            <button onClick={() => setEredmeny(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center bg-black/50 text-white rounded-full font-bold z-[210]">✕</button>
            <div className="overflow-y-auto flex-1">
              <div className="bg-orange-600 p-8 md:p-10 text-center text-white relative">
                <h2 className="text-3xl md:text-5xl font-black tracking-tighter italic">{eredmeny.modell}</h2>
              </div>
              <div className="p-8 md:p-12 text-center">
                <p className="text-gray-700 font-bold leading-relaxed text-xl mb-10">{eredmeny.indoklas}</p>
                <div className="space-y-4">
                  <a href={eredmeny.link} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-4 px-8 py-5 bg-[#ff5a00] text-white rounded-2xl font-black text-xl uppercase tracking-widest hover:bg-orange-600 shadow-xl transition-all">
                    Megtekintem a terméket
                  </a>
                  {eredmeny.kereskedo && (
                    <div className="p-6 bg-gray-50 rounded-[2rem] border-2 border-gray-100 text-left shadow-inner mt-4">
                      <p className="text-[10px] font-black text-orange-600 uppercase mb-2 italic">📍 Legközelebbi szakkereskedő</p>
                      <h3 className="font-black text-gray-950 text-2xl">{eredmeny.kereskedo.nev}</h3>
                      <p className="text-sm text-gray-500 font-bold mt-1">
                        {eredmeny.kereskedo.cim} {eredmeny.tavolsag !== null && <span className="text-orange-600 ml-2">(~{eredmeny.tavolsag} km)</span>}
                      </p>
                      <div className="grid grid-cols-2 gap-3 mt-5">
                          <a href={`https://www.google.com/maps/dir/?api=1&destination=${eredmeny.kereskedo.lat},${eredmeny.kereskedo.lon}`} target="_blank" rel="noreferrer" className="bg-white border border-gray-200 py-3 rounded-xl text-center text-xs font-black uppercase tracking-widest">Útvonal</a>
                          <a href={`tel:${eredmeny.kereskedo.telefon}`} className="bg-gray-950 text-white py-3 rounded-xl text-center text-xs font-black uppercase tracking-widest">Hívás</a>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {isAnimating && (
        <div className="fixed inset-0 z-[100] bg-gray-950/90 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center">
          <h2 className="text-3xl md:text-5xl font-black text-white uppercase tracking-[0.3em] italic animate-pulse">Elemzés folyamatban...</h2>
          <p className="text-orange-500 font-black mt-6 text-6xl italic">{progress}%</p>
        </div>
      )}
    </div>
  );
